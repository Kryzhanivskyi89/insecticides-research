...
const ResultsTable = ({ experiment = {}, activityData = [], conclusion = "", onConclusionChange }) => {
  ...
  const controlDays = Array.isArray(control.days) ? control.days : [];

  const allDaysSet = new Set();
  controlDays.forEach(d => d?.day && allDaysSet.add(d.day));
  samplesData.forEach(sample => {
    sample.repetitions?.forEach(rep => {
      rep.days?.forEach(d => d?.day && allDaysSet.add(d.day));
    });
  });
  const days = Array.from(allDaysSet).sort((a, b) => parseInt(a) - parseInt(b));
    ...
    return (
      <tr className={styles.rowControl} key="control">
        ...
        {hasBitoxDry && days.map(day => (
          <td className={styles.cell} key={`ctrl-day-${day}`}>{valuesByDay[day] ? average(valuesByDay[day]) : "-"}</td>
        ))}
        {days.map(day => (
    ...
    return samplesData.map(sample => {
      ...
      if (bitoxDry) {
        days.forEach(day => {
          ...
          avgValuesByDay[day] = valuesForDay;
        });
      }

      return (
        <tr className={styles.rowSample} key={sample.sampleId}>
          <td className={styles.cell}>{sample.name || "-"}<br /><small> {sample.base || "-"}</small></td>
          ...
          {hasBitoxDry && days.map(day => (
            <td className={styles.cell} key={`val-${sample.sampleId}-${day}`}>
              {bitoxDry ? average(avgValuesByDay[day]) : "-"}
            </td>
          ))}
          {days.map(day => {
            const activityEntry = sampleActivity?.activities?.find(a => a.day === day);
            const activityValue = activityEntry?.activity !== undefined ? `${activityEntry.activity}%` : "-";
            return (
  ...
  return (
    <div className={styles.resultsTableWrapper}>
      ...
      <table className={styles.table}>
        <thead>
          <tr>
            <th className={styles.header}>Препарат / Основа</th>
            <th className={styles.header}>Концентрація</th>
            {hasBitoxDry && <th className={styles.header}>До обробки</th>}
            {hasBitoxDry && days.map(day => (
              <th className={styles.header} key={`header-val-${day}`}>Чисельність {day} день</th>
            ))}
            {days.map(day => (
              <th className={styles.header} key={`header-act-${day}`}>Активність {day} день (%)</th>
            ))}
          </tr>
        </thead>
        <tbody>
      ...
      <div className={styles.conclusionBlock}>
        <label className={styles.conclusionLabel}>Висновок:</label>
        <textarea
...
//       return (
//         <tr className={styles.rowSample} key={sample.sampleId}>
//           <td className={styles.cell}>{sample.name || "-"}</td>
//           <td className={styles.cell}>{sample.concentration || "-"}</td>
//           {hasBitoxDry && (
//             <td className={styles.cell}>{bitoxDry ? average(beforeValues) : "-"}</td>
//           )}
